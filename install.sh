...
if [ -d "Nystem" ]; then
  echo "Nystem exists, backing up to .config/nystem-backups folder."
  if [ -d ".config/nystem-backups" ]; then
    echo "Moving current version of Nystem to backups folder."
    mv "$HOME"/nystem .config/nystem-backups/"$backupname"
    sleep 1
  else
    echo "Creating the backups folder & moving Nystem to it."
    mkdir -p .config/nystem-backups
    mv "$HOME"/Nystem .config/nystem-backups/"$backupname"
    sleep 1
  fi
else
  echo "Thank you for choosing Nystem."
  echo "I hope you find your time here enjoyable!"
fi

echo "-----"

echo "Cloning & Entering Nystem Repository"
git clone  https://github.com/quocjq/Nystem.git
cd Nystem || exit
...
sudo nixos-rebuild switch --flake ~/Nystem/#${profile}
